% Program: charalambous.m
% Title: Charalambous minimax algorithm.
% Description: Implements the Charalambous minimax algorithm
% (Algorithm 8.2) for the design of stable lowpass, highpass, 
% bandpass, and bandstop IIR digital filters.
% Theory: See Practical Optimization Sec. 8.3.
% This MATLAB function requires the following MATLAB functions:
%   bfgs_charalambous.m; f_charalambous.m; g_charalambous.m;
%   get_ksi_lambda.m; inex_lsearch.m; initialize.m,
%   stabilize.m
% Input:
...
%   epsi1: termination tolerance for Algorithm 8.2
...
%   Example:
% Applying Algorithm 8.2, design a 6th-order lowpass stable
...
%   epsi1 = 1e-7
%   epsi2 = 1e-7
%   [a,b,as,bs,ep_max,ea_max,k]=charalambous(typ,f,N,K,epsi1,epsi2,x_ini)
% or
%   [a,b,as,bs,ep_max,ea_max,k] = charalambous(typ,f,N,K,epsi1,epsi2)
% =======================================================================
function[a,b,as,bs,ep_max,ea_max,k]=...
   charalambous(typ,f,N,K,epsi1,epsi2,x_ini)
disp(' ')
disp('Program charalambous.m')
warning off
...
x_ini
while er >= epsi1,
    x = bfgs_charalambous(x_ini,N,K,ksi,M0,lamd,omi,epsi2);
    [ksi,lamd] = get_ksi_lambda(x,M0,N,K,ksi,lamd,omi);
    a = x(1:(N+1));
    b = [1; x((N+2):end)];
    H = freqz(a,b,omi);
    emax1 = max(abs(abs(H) - M0));
    er = abs(emax1 - emax);
    kw = k + 1;
